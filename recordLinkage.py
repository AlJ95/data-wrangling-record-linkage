...
results = pd.DataFrame([], columns=[
    ...
    "blocking_func",
    "comp_func",
    "classification_func", "sim_threshold", "weights",
    "reducation_ratio", "pairs_completeness", "pairs_quality",
    "TP", "FP", "FN", "TN",
    "accuracy", "precision", "recall", "effective_recall", "f_measure", "effective_f_measure",
    "loading_time", "blocking_time", "comparison_time", "classification_time"
])
for name in ["clean", "little-dirty", "very-dirty"]:
    for records in [1_000, 10_000, 100_000]:
        for comp_func in [
            ...
            # comp_func = "approx_comp_funct_list"
            for sim_threshold in [0.5, 0.7]:
                ...
                print(f"Name:{name}\nRecords:{records}")
                print("#############################################")
                for blocking_func in [blocking.phoneticBlocking, blocking.slkBlocking]:
                    for classification_func in [
                        # classification.thresholdClassify, classification.minThresholdClassify,
                        #                         classification.weightedSimilarityClassify,
                                                classification.supervisedMLClassify
                                                ]:

                        if records > 100_000 or blocking_func == blocking.phoneticBlocking:
                            continue
                        ...
                        if comp_func == "approx_comp_funct_list":
                            comp_func_list = approx_comp_funct_list
                        else:
                            comp_func_list = [(comp_func, 1, 1),  # First name
                                              (comp_func, 2, 2),  # Middle name
                                              (comp_func, 3, 3),  # Last name
                                              (comp_func, 8, 8),  # Suburb
                                              (comp_func, 7, 7),  # Address
                                              (comparison.exact_comp, 10, 10),  # State
                                              ]
                        # =============================================================================
                        #
                        ...
                        #
                        if blocking_func == blocking.phoneticBlocking:
                        ...
                        if blocking_func == blocking.slkBlocking:
                        ...
                        # sim_threshold = 0.7
                        # #
                        if classification_func == classification.thresholdClassify:
                            class_match_set, class_nonmatch_set = \
                        ...
                        if classification_func == classification.minThresholdClassify:
                            class_match_set, class_nonmatch_set = \
                        ...
                        if classification_func == classification.weightedSimilarityClassify:
                        ...
                        # weight_vec = [2.0, 1.0, 2.0, 2.0, 2.0, 1.0]
                            weight_vec = [vec1 * vec2 for vec1, vec2 in zip(weight_vec, [1.0, 0.05, 2.0, 1., 1., 0.05])]
                        ...
                        #
                        if classification_func == classification.supervisedMLClassify:
                            class_match_set, class_nonmatch_set = \
                        ...
                        TP, FP, FN, TN = linkage_result
                        effective_recall = recall / pc
                        effective_fmeasure = 2 * precision * effective_recall / (precision + effective_recall)
                        results = pd.concat([
                          results,
                            pd.DataFrame([[
                                name, records, blocking_func.__name__, ",".join([tup[0].__name__ for tup in comp_func_list]),
                                classification_func.__name__, sim_threshold,
                                weight_vec if classification_func == classification.weightedSimilarityClassify else None,
                                rr, pc, pq, TP, FP, FN, TN, accuracy, precision, recall, effective_recall, fmeasure,
                                effective_fmeasure, loading_time, blocking_time, comparison_time, classification_time
                            ]], columns=[
                                "dataset",
                                "records",
                                "blocking_func",
...
results.to_excel("Results_Record_Linkage2.xlsx")